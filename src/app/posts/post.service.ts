...
export class PostService {
  ...
  addPost(newPost: Post, image: File): void {
    ...
    this.http
      .post<{ message: string; post: Post}>(
        'http://13.51.146.142:3000/api/posts',
        postData
      )
      .subscribe({
        next: (responseData) => {
  ...
  deletePost(id: string): void {
    this.http
      .delete(`http://13.51.146.142:3000/api/posts/${id}`)
      .subscribe((responseData) => {
  ...
  editPost(currentPost: Post): void {
    if(typeof currentPost.imagePath !== 'string') {
      ...
      this.http
      .put<{message: string}>('http://13.51.146.142:3000/api/posts/' + currentPost.id, postData)
      .subscribe((responseData) => {
        this.router.navigate([''])
      });
    ...
    this.http
      .put<{message: string}>('http://13.51.146.142:3000/api/posts/' + currentPost.id, currentPost)
      .subscribe((responseData) => {
        this.router.navigate([''])
      });
  ...
  getPostByIdDb(id: string) {
    ...
    return this.http.get<{message: string, post: Post}>(`http://13.51.146.142:3000/api/posts/${id}`);
  }
}
